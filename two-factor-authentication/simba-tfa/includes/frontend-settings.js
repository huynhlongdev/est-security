...
jQuery(function ($) {
  $(
    ".tfa_settings_form input, .tfa_settings_form textarea, .tfa_settings_form select"
  ).on("change", function () {
  ...
  $(".simbatfa_settings_save").on("click", function () {
    $.blockUI({
      message:
        '<div style="margin: 8px;font-size:150%;">' +
        simba_tfa_frontend.saving +
        "</div>",
    });
    ...
    $.each(
      $(".tfa_settings_form input[type=checkbox]").filter(function (idx) {
