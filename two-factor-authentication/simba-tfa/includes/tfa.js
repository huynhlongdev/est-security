jQuery(function ($) {
  ...
  function process_user_tfa_enabled_check_results(form, response) {
    if (true === response.status) {
      ...
      var user_can_trust =
        response.hasOwnProperty("user_can_trust") && response.user_can_trust
          ? true
          : false;

      var user_already_trusted =
        response.hasOwnProperty("user_already_trusted") &&
        response.user_can_trust
          ? true
          : false;
  ...
  function check_and_possibly_show_otp_field(form, only_cache_the_results) {
    ...
    if ("object" === typeof username_requires_otp[username]) {
      if (!only_cache_the_results) {
        ...
        return process_user_tfa_enabled_check_results(
          $(form),
          username_requires_otp[username]
        );
      }
    ...
    var $submit_button = $(form)
      .find(
        'input[name="wp-submit"], input[type="submit"], button[type="submit"]'
      )
      .first();

    if (
      simba_tfasettings.hasOwnProperty("spinnerimg") &&
      $(".simbaotp_spinner").length === 0
    ) {
      ...
      $submit_button.after(
        '<img class="simbaotp_spinner" src="' +
          simba_tfasettings.spinnerimg +
          '" style="' +
          styling +
          '">'
      );
  ...
  function show_otp_field(form, user_can_trust, user_already_trusted) {
    ...
    user_can_trust =
      "undefined" == typeof user_can_trust ? false : user_can_trust;
    user_already_trusted =
      "undefined" == typeof user_already_trusted ? false : user_already_trusted;

    if (
      "https:" != window.location.protocol &&
      "localhost" !== location.hostname &&
      "127.0.0.1" !== location.hostname &&
      /^\.localdomain$/.test(location.hostname)
    ) {
    ...
    var form_is_gravity_forms =
      "object" == typeof window["gform_gravityforms"] &&
      "undefined" !== typeof $(form).attr("id") &&
      "gform_" === $(form).attr("id").substring(0, 6);

    // This is used just for applying similar styling (via adding structure/CSS classes)
    var form_is_ultimate_member =
      $(form).find(".um-row").length > 0 ? true : false;

    // This is used just for applying styling if .js-login-form class exists inside form
    var form_is_login_form =
      $(form).find(".js-login-form").length > 0 ? true : false;
    ...
    $submit_button = $(form).find(
      'input[name="wp-submit"], input[name="Submit"], input[name="submit"]'
    );
    ...
    if (!form_retain_existing_elements) {
      ...
      $submit_button
        ...
        .first()
        .find(
          "p, .impu-form-line-fr, .tml-field-wrap, .user-pass-wrap, .elementor-field-type-text, .elementor-field-type-submit, .elementor-remember-me, .bbp-username, .bbp-password, .bbp-submit-wrapper, .gform_body, .um-row, .um-button, .js-login-form, .rmrow"
        )
        .each(function (i) {
    ...
    if (user_already_trusted) {
      html +=
        '<br><span class="simbaotp_is_trusted">' +
        simba_tfasettings.is_trusted +
        "</span>";
    } else {
      if (form_is_ultimate_member) {
        html +=
          '<div class="um-field um-field-text um-field-type_text"><div class="um-field-label">';
      }
      ...
      html +=
        simba_tfasettings.otp +
        '</label><input type="text" name="two_factor_code" id="simba_two_factor_auth" autocomplete="off" data-lpignore="true"';
      ...
      if (user_can_trust) {
        html +=
          '<input type="checkbox" name="simba_tfa_mark_as_trusted" id="simba_tfa_mark_as_trusted" value="1"><label for="simba_tfa_mark_as_trusted">' +
          simba_tfasettings.mark_as_trusted +
          "</label>";
      }
    }
    ...
    if (!form_is_gravity_forms) {
      ...
      html +=
        '"><input id="tfa_login_btn" class="button button-primary button-large';

      if (form_is_ultimate_member) {
        html += " um-button";
      }

      if (form_is_login_form) {
        html += " c-btn-rg hover:bg-main focus:bg-main";
      }

      html += '" type="submit" ';
      if (
        "undefined" !== typeof submit_button_name &&
        "" != submit_button_name
      ) {
        html += 'name="' + submit_button_name + '" ';
      }
  ...
  var form_submit_handler = function (e) {
    ...
    if (
      0 == $(form).find("#simba_two_factor_auth").length &&
      check_and_possibly_show_otp_field(form)
    ) {
      if (form_is_gravity_forms) {
  ...
  $(simba_tfasettings.login_form_selectors)
    .find(get_username_identifiers())
    .on("blur", function () {
